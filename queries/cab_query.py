def get_ecom_query(merchant_name, parser_list):
    ...
    ROUND(
        100.0 * COUNT(DISTINCT CASE WHEN order_other_fee IS NOT NULL THEN order_foxid END) 
        / NULLIF(COUNT(DISTINCT order_foxid), 0), 
        2
    ) AS completion_order_other_fee,
    ROUND(
    ...
    ROUND(
        100.0 * COUNT(DISTINCT CASE WHEN order_item_departure_address IS NOT NULL THEN order_foxid END) 
        / NULLIF(COUNT(DISTINCT order_foxid), 0), 
        2
    ) AS completion_order_item_departure_address,
    ROUND(
        100.0 * COUNT(DISTINCT CASE WHEN order_item_arrival_address IS NOT NULL THEN order_foxid END) 
        / NULLIF(COUNT(DISTINCT order_foxid), 0), 
        2
    ) AS completion_order_item_arrival_address,
    ROUND(
        100.0 * COUNT(DISTINCT CASE WHEN order_item_total_price_paid IS NOT NULL THEN order_foxid END) 
        / NULLIF(COUNT(DISTINCT order_foxid), 0), 
        2
    ) AS completion_order_itemtotal_price_paid,
    ROUND(
    ...
    ROUND(
        100.0 * COUNT(DISTINCT CASE WHEN order_order_date IS NOT NULL THEN order_foxid END) 
        / NULLIF(COUNT(DISTINCT order_foxid), 0), 
        2
    ) AS completion_order_order_date,
    ROUND(
    ...
    ROUND(
        100.0 * COUNT(DISTINCT CASE WHEN order_promo IS NOT NULL THEN order_foxid END) 
        / NULLIF(COUNT(DISTINCT order_foxid), 0), 
        2
    ) AS completion_order_promo,
    ROUND(
    ...
    ROUND(
        100.0 * COUNT(DISTINCT CASE WHEN product_class IS NOT NULL THEN order_foxid END) 
        / NULLIF(COUNT(DISTINCT order_foxid), 0), 
        2
    ) AS completion_product_class,
