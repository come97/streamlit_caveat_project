def get_food_delivery_query(merchant_name, parser_list):
    ...
    ROUND(
        100.0 * COUNT(DISTINCT CASE WHEN order_item_delivery_address IS NOT NULL THEN order_foxid END) 
        / NULLIF(COUNT(DISTINCT order_foxid), 0), 
        2
    ) AS completion_order_item_delivery_address,
    ROUND(
    ...
    ROUND(
        100.0 * COUNT(DISTINCT CASE WHEN order_delivery_fee IS NOT NULL THEN order_foxid END) 
        / NULLIF(COUNT(DISTINCT order_foxid), 0), 
        2
    ) AS completion_order_item_delivery_fee,
    ROUND(
