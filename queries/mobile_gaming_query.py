def get_mobile_gaming_query(merchant_name, parser_list):
    ...
    ROUND(
        100.0 * COUNT(DISTINCT CASE WHEN order_tax IS NOT NULL THEN order_foxid END) 
        / NULLIF(COUNT(DISTINCT order_foxid), 0), 
        2
    ) AS completion_order_tax,
    ROUND(
    ...
    ROUND(
        100.0 * COUNT(DISTINCT CASE WHEN order_item_option_1 IS NOT NULL THEN order_foxid END) 
        / NULLIF(COUNT(DISTINCT order_foxid), 0), 
        2
    ) AS completion_order_item_option_1
